...
if(edicion==='amplia'){const raiz=path.join(__dirname,'..'),dir=path.join(raiz,'tools','ilustraciones'),fu=JSON.parse(fs.readFileSync(path.join(dir,'fuentes.json'),'utf8')),datos=fs.readFileSync(path.join(dir,'ilustraciones_datos.py'),'utf8');
  ...
  // y las barcas, las mercancías y las postales pintadas: cada una con su dibujo en tools/dibujos/barcas/, bienes/ o postales/
  let barcas=0,bienes=0,postales=0;
  for(const [archivo,carpeta] of [['barcas.js','barcas'],['bienes.js','bienes'],['postales.js','postales']])for(const l of fs.readFileSync(path.join(raiz,'src','data',archivo),'utf8').split('\n')){const k=(l.match(/^([a-z]+):[{[]/)||[])[1];if(!k)continue;
    if(!fs.existsSync(path.join(raiz,'tools','dibujos',carpeta,k+'.py')))throw 'pintada sin su dibujo: '+carpeta+'/'+k;const n=(l.match(/data:image\/webp/g)||[]).length;if(carpeta==='barcas')barcas++;else if(carpeta==='bienes')bienes+=n;else postales+=n}
  console.log('barcas: '+barcas+' pintadas · mercancías: '+bienes+' pintadas · postales: '+postales+' pintadas')}
