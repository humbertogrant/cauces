...
for(const r of RUTAS){
  ...
  for(const c of r.paradas){for(const k of ['nombre','pais','pos','imagen','dato','escena','fecha'])if(c[k]==null)throw r.id+' '+c.nombre+' falta '+k;if(c.imagen.length>260)console.log('imagen larga',c.nombre,c.imagen.length);for(const e of c.escena)if(!pictoOk(e))throw 'pictograma '+e+' en '+c.nombre;
