...
function dichoActual(){if(S.pantalla==='quiz')return S.quiz&&S.quiz.resp!=null?S.quiz.dicho:null;if(S.pantalla!=='rio')return null;const r=rioPor(S.rio);
  ...
  if(S.tab==='trazar')return S.tra&&S.tra.dicho;
...
const num=s=>`<span class="num">${s}</span>`;/* números en Oswald */
...
/*@vocab:rio*/const al=s=>('a '+s).replace(/^a el /,'al ');/* «a» ante el marEn, con contracción: «al golfo Pérsico», «a la bahía de Bengala» */ /*@fin:rio*/
const VOCAB={
  /*@vocab:rio*/
  rio:{tipo:'río',tipos:'ríos',Tipos:'Ríos',
    ...
    llegarFin:'Llegar al mar',avanzar:'Zarpar a',bajar:'Bajar',
    ...
    selloNota:'Cada sello se gana llegando a una ciudad sabiendo adónde ibas; se vuelve dorado cuando la recitás de memoria y además la recordás días después.',
    ...
    dominas:'Reconocés todo el cauce: ahora recitalo de memoria.',conCalma:'Volvé a bajar el río con calma; el repaso te lo va a recordar.',
    ...
    cabecera:r=>`${esc(r.region)} · ${num(km(r.longitud)+' km')} hasta ${esc(r.fin.en)}`,tarjeta:r=>num(km(r.longitud)+' km'),
    kickerInicio:r=>`Nacimiento · km ${num('0 de '+km(r.longitud))}`,kickerParada:(r,p,n,c)=>`Parada ${num(p+' de '+n)} · km ${num('≈'+km(c.km)+' de '+km(r.longitud))}`,kickerFin:r=>`Desembocadura · km ${num(km(r.longitud))}`,
    tramo:(r,c,prev)=>prev?`${num(km(c.km-prev.km)+' km')} desde ${esc(prev.nombre)}`:c.km?`${num(km(c.km)+' km')} desde la fuente`:'en la fuente misma',
    llegada:r=>`Después de ${km(r.longitud)} km, el ${r.nombre} llega ${al(r.fin.en)}.`,contame:cx=>'Contame más: la '+cx.titulo.toLowerCase(),
    vozInicio:r=>`Nacimiento: ${r.inicio.nombre}.`,vozParada:(p,n,c)=>`Parada ${p} de ${n}: ${c.nombre}, ${c.pais}.`,vozFin:r=>`Desembocadura: ${r.fin.nombre}. Después de ${r.longitud} kilómetros, el ${r.nombre} llega ${al(r.fin.en)}.`,
  ...
  itinerario:{tipo:'viaje',tipos:'viajes',Tipos:'Viajes',
    inicio:'Partida',fin:'Regreso',extremoInicio:'Partida',extremoFin:'Regreso',entreInicio:'la partida',entreFin:'el regreso',Parada:'Etapa',proxima:'¿Hacia dónde sigue el viaje?',
    ...
    selloNota:'Cada sello se gana llegando a una etapa sabiendo adónde ibas; se vuelve dorado cuando la recitás de memoria y además la recordás días después.',
    ...
    ordenBien:'Sin errores: el camino ya es tuyo.',ordenMal:e=>`${e} errores: volvé a recorrer el viaje con la frase en mano.`,tocaEnOrden:n=>`Tocá las ${n} etapas en orden, de la partida al regreso.`,tocaTramos:n=>`Tocá los ${n} tramos en orden, de la partida al regreso.`,trazar:'Trazar',trazaKicker:'Trazar · la forma del viaje',trazaPregunta:(i,n,prev)=>`Tocá en el mapa la etapa ${i} de ${n}: ¿adónde siguió el viaje después de ${esc(prev)}?`,trazaNo:'Ahí no. Mirá el mapa otra vez: ¿hacia dónde siguió?',trazaFin:(ok,n)=>`${ok} de ${n} etapas a la primera`,
  ...
  travesia:{tipo:'travesía',tipos:'travesías',Tipos:'Viajes',
    inicio:'Zarpe',fin:'Regreso',extremoInicio:'Zarpe',extremoFin:'Regreso',entreInicio:'el zarpe',entreFin:'el regreso',Parada:'Escala',proxima:'¿Hacia dónde sigue la travesía?',
    ...
    selloNota:'Cada sello se gana llegando a una escala sabiendo adónde ibas; se vuelve dorado cuando la recitás de memoria y además la recordás días después.',
    recorre:'Recorré un viaje o jugá un reto: lo que hagás vuelve cuando toque.',recorreFin:'No hay nada pendiente todavía. Recorré un viaje hasta el regreso o jugá un reto: lo que hagás entra al repaso y vuelve cuando toque.',
    ...
    ordenBien:'Sin errores: la ruta ya es tuya.',ordenMal:e=>`${e} errores: volvé a navegar con la frase en mano.`,tocaEnOrden:n=>`Tocá las ${n} escalas en orden, del zarpe al regreso.`,tocaTramos:n=>`Tocá los ${n} tramos en orden, del zarpe al regreso.`,trazar:'Trazar',trazaKicker:'Trazar · la forma de la travesía',trazaPregunta:(i,n,prev)=>`Tocá en el mapa la escala ${i} de ${n}: ¿adónde siguió la travesía después de ${esc(prev)}?`,trazaNo:'Ahí no. Mirá el mapa otra vez: ¿hacia dónde siguió?',trazaFin:(ok,n)=>`${ok} de ${n} escalas a la primera`,
    cabecera:r=>`${esc(r.region)} · de ${esc(r.inicio.nombre)} a ${esc(r.fin.en)}`,tarjeta:r=>esc(r.region),
...
const textoAltura=(r,i)=>{const a=alturaParada(r,i);return a==null?'':a<0?` · el río va ${num(km(-a)+' m')} bajo el nivel del océano`:a<5?' · el río ya va al nivel del mar':` · el río pasa a ${num(km(a)+' m')} sobre el mar`};
...
function lectura(){const r=S.pantalla!=='inicio'&&S.pantalla!=='pasaporte'&&S.rio&&rioPor(S.rio);if(!r)return[];const V=r.vocab,n=r.paradas.length,m=r.companero,h=modo()==='historia',L=[];
  ...
  if(S.tab==='ordenar'){const o=S.orden,U=unidades(r);di(o.dicho);if(!o.listo)opciones(o.pool.map(i=>U[i].nombre),r.tramos?V.tocaTramos(U.length):V.tocaEnOrden(n));return L}
  if(S.tab==='trazar'){const T=S.tra;di(T.dicho);if(!T.listo)di(V.trazaPregunta(T.i+1,n,T.i?r.paradas[T.i-1].nombre:r.inicio.nombre));return L}
  ...
  const guia=()=>{const g=S.guias[p];if(g&&g.resp==null&&p<n)opciones(g.opciones.map(i=>rotuloGuia(r,i)),V.proxima)};
...
const nuevoP=()=>({cards:{},vistos:{},sellos:{},etapas:{}});
...
const selloOro=(r,i)=>{const c=P.cards['ciudad:'+r.id+':'+i];return !!(c&&c.box>=3&&c.rec)};/* caja ≥ 3 y recitada de memoria alguna vez */
...
function importarTexto(txt){let d=null;try{d=JSON.parse(txt)}catch(e){}
  ...
  if(!PERFILES.lista.includes(n))PERFILES.lista.push(n);PERFILES.activo=n;guardarPerfiles();P={cards:p.cards||{},vistos:p.vistos||{},tesoro:p.tesoro||0,modo:p.modo,voz:p.voz,sellos:p.sellos||{},etapas:p.etapas||{},dia:p.dia};guardar();S.perfilNuevo=false;S.aviso=`Progreso de ${n} cargado.`;render(false);return true}
...
/* Leitner con dos clases de evidencia: reconocer (elegir entre opciones: guía, retos, repaso, eventos) sube una tarjeta solo
   hasta la caja 3; recordar (Recitar: decir la parada de memoria) deja la marca `rec` (1 con pista, 2 sin pista) y abre las
   cajas 4 y 5. Una tarjeta solo sube cuando está vencida: acertarla tres veces la misma tarde cuenta una sola vez; fallarla
   la baja a 0 y borra `rec`. Así nada se muestra consolidado por haberlo reconocido recién. */
function marcar(id,ok,via,nivel){const c=P.cards[id]||{box:0,due:0},t=Date.now();if(c.nuevo){delete c.nuevo;diaHoy().nuevas++}
...
/*@fin:itinerario*/
/*@vocab:itinerario*/
/* ---------- Trazar: reconstruir la forma del viaje tocando en el mapa la siguiente etapa (recuerdo espacial; en los viajes
   reemplaza a Recitar) y la ficha de la portada que compara los viajes en escala ---------- */
function iniciarTrazar(r){S.tra={i:0,errores:0,malos:0,fallo:null,listo:false,viaje:false,dicho:azar(VZ().recitarInicio),premio:0,ok:[]}}
function trazarEn(i){const r=rioPor(S.rio),T=S.tra;if(!T||T.listo||S.tab!=='trazar')return;const n=r.paradas.length;
  if(i===T.i){const bien=T.malos===0;T.ok.push(bien);marcar('ciudad:'+r.id+':'+i,bien,'recordar',1);T.i++;T.malos=0;T.fallo=null;T.viaje=true;T.dicho=azar(VZ().ordenBien);audio.ping(1046);
    if(T.i>=n){T.listo=true;T.dicho=VZ().ordenFin[T.errores===0?0:T.errores<=2?1:2];if(S.eco&&T.errores<=2){T.premio=T.errores===0?5:2;P.tesoro=(P.tesoro||0)+T.premio;T.dicho+=` +${T.premio} monedas al tesoro.`}if(T.errores<=2)hecha(r,'trazar');guardar()}}
  else{T.errores++;T.malos++;T.fallo=i;T.dicho=azar(VZ().ordenMal);setTimeout(()=>{if(S.tra===T&&T.fallo===i){T.fallo=null;render(false)}},600)}
  render(false)}
function renderTrazar(r){const V=r.vocab,T=S.tra,n=r.paradas.length,prev=T.i?r.paradas[T.i-1].nombre:r.inicio.nombre;
  const seq=`<div class="secuencia"><span class="extremo">${V.extremoInicio}</span>${r.paradas.map((c,i)=>i<T.i?`<span class="puesto${T.ok[i]?'':' no'}">${esc(c.nombre)}</span>`:'<span class="hueco">—</span>').join('')}<span class="extremo">${V.extremoFin}</span></div>`;
  const cuerpo=T.listo?`<div class="nota${T.errores>2?' no':''}">${V.trazaFin(T.ok.filter(Boolean).length,n)}${T.premio?` · +${T.premio} monedas al tesoro`:''}.</div>`:`<p class="pregunta">${V.trazaPregunta(T.i+1,n,prev)}</p>${T.fallo!=null?`<div class="nota no">${V.trazaNo}</div>`:''}`;
  return `${globo(r,T.dicho)}<p class="kicker">${botonVoz()}${V.trazaKicker}</p>${cuerpo}${seq}`}
/* el mapa en Trazar: solo el camino ya trazado, todas las etapas como puntos tocables y sin nombre hasta acertarlas */
...
function siguiente(){const Q=S.quiz;Q.i++;Q.resp=null;Q.dicho=azar(VZ().pregunta);if(Q.i>=Q.qs.length&&Q.modo==='rio'&&S.rio&&nivelQuiz(Q)<=1)hecha(rioPor(S.rio),'preguntar');render()}
...
function globo(r,texto,extra,animo){const m=r&&r.companero;if(!m||!texto)return '';GLOBO=true;const pj=typeof ilustracion==='function'&&ilustracion(m,animo||S.animo||'normal','cabeza');/* la ilustración con su ánimo, en primer plano, si la edición la trae */
  return `<div class="globo${extra?' '+extra:''}"><span class="emo${pj?' retrato':''}" aria-hidden="true">${pj||animal(m)}</span><div class="dice"><b>${esc(m.nombre)}:</b> ${esc(texto)} ${htmlVoz()}</div></div>`}
/*@amplia*/
/* ilustraciones (ILUSTRACIONES, src/data/ilustraciones.js, generado por tools/ilustraciones.py): un dibujo por animal sobre un contorno
   ...
   pone caraDe según el ánimo (normal parpadea; alegre, pensativo, sorpresa, dormido). La cara corresponde a la vista del contorno: de
   perfil, un solo ojo; de frente o desde arriba (c.vista 'frente' | 'arriba': el cangrejo, la tortuga), los dos, en ojo y ojo2, iguales y con la pupila al centro.
...
function caraDe(c,a,inc){const k=c.k||2,kb=c.kb||k*2,g=c.giro||0,pb=c.pb||1,av=c.vista?0:1,[ex,ey]=c.ojo,f=v=>+v.toFixed(2),w=f(k*0.3),
  ...
  const ojo={normal:blanco(k*1.15)+pupila(k*0.2*av,0,k*0.72)+`<circle class="cuerpo parpado" style="stroke-width:${w}" r="${f(k*1.3)}"/>`,
    alegre:linea(`M${f(-k*1.1)} ${f(k*0.35)}Q0 ${f(-k*1.3)} ${f(k*1.1)} ${f(k*0.35)}`,k*0.42),
    sorpresa:blanco(k*1.45)+pupila(k*0.15*av,0,k*0.6),
    dormido:linea(`M${f(-k*1.1)} ${f(-k*0.2)}Q0 ${f(k*1.1)} ${f(k*1.1)} ${f(-k*0.2)}`,k*0.42),
    pensativo:blanco(k*1.15)+pupila(k*0.35*av,-k*0.25,k*0.62)+linea(`M${f(-k*1.2)} ${f(-k*1.9)}q${f(k*1.2)} ${f(-k*0.8)} ${f(k*2.4)} ${f(-k*0.1)}`,k*0.38)}[a]||'';
...
function laminaNave(r){const b=typeof BARCAS!=='undefined'&&BARCAS[r.id];if(!b)return '';const [x,y,w,h]=b.v.split(' ');/* la embarcación propia de la ruta, pintada (la falúa del Nilo, la caja de Ulm del Danubio), en lugar del glifo chico; `f`: el agua de vectores, detrás */
  return `<div class="lamina"><svg class="barca-pintada" viewBox="${b.v}" aria-hidden="true">${b.f||''}<image href="${b.i}" x="${x}" y="${y}" width="${w}" height="${h}"/></svg></div>`}
function bienPintado(r,gi){const b=typeof BIENES!=='undefined'&&BIENES[r.id],i=b&&b[gi];return i?`<img class="bien" src="${i}" alt="">`:''}/* la mercancía pintada, en las rutas que la tienen; si no, el icono de línea */
function postal(rid,i,cls){const p=typeof POSTALES!=='undefined'&&POSTALES[rid],s=p&&p[i];return s?`<img class="escena pintada${cls?' '+cls:''}" src="${s}" alt="">`:''}/* la postal pintada del río (tools/dibujos/postales): i = 0 el nacimiento, 1…n cada parada, n+1 el mar; donde no hay, queda la de pictogramas */
...
function renderInicio(){
  ...
  const grupos=JUEGO.grupos.map(g=>{const rs=RUTAS.filter(g.filtro).sort(porLargo);let cuerpo=cuadricula(rs);
    ...
    return `<div class="grupo${g.clase?' '+g.clase:''}"><h3>${g.titulo}</h3>${g.nota?`<p class="fnota">${g.nota}</p>`:''}${g.antes||''}${cuerpo}</div>`}).join('\n');
  return `${renderHoy()}
...
function renderDescender(r){if(S.evento)return renderEvento(r);const V=r.vocab;const n=r.paradas.length,p=S.paso;
  if(p===0)return `${globo(r,S.dicho||(r.companero&&r.companero.hola))}<p class="kicker">${botonVoz()}${V.kickerInicio(r)}${r.perfil?` · ${km(r.perfil[0][1])} m sobre el mar`:''}</p><h2>${esc(r.inicio.nombre)}</h2><div class="ficha recuerdo">/*@amplia*/${postal(r.id,0)||`/*@fin:amplia*/${escena(r.inicio.escena)}/*@amplia*/`}/*@fin:amplia*/<div class="cuerpo"><p>${esc(r.inicio.nota)}</p></div></div>${r.vehiculo?`<div class="nave">/*@amplia*/${laminaNave(r)||`/*@fin:amplia*/<svg class="barca mini" viewBox="-15 -15 30 20" aria-hidden="true">${glifo(r.vehiculo.tipo)}</svg>/*@amplia*/`}/*@fin:amplia*/<div class="fkicker">${V.tuVehiculo}</div><div class="ntit">${esc(r.vehiculo.nombre)}</div><p>${esc(r.vehiculo.desc)}</p><div class="fnota">${S.eco?V.zarpas+(r.fantasma!=null?` El ${V.mercader} suele llegar con ${r.fantasma}; a ver si le ganás.`:''):esc(r.vehiculo.zarpe)}</div></div>`:''}${typeof fichaAnimal==='function'?fichaAnimal(r):''}${mas(V.contame(r.contexto[0]),`<p>${esc(r.contexto[0].texto)}</p>`,r.contexto[0].titulo)}${bloqueFrase(r,true)}`;
  if(p<=n){const c=r.paradas[p-1];
    const prev=p>1?r.paradas[p-2]:null;
    return `${globo(r,S.dicho||(r.companero&&r.companero.paradas[p-1]))}<p class="kicker">${botonVoz()}${V.kickerParada(r,p,n,c)}</p><h2>${esc(c.nombre)}</h2><div class="pais">${esc(c.pais)} · ${V.tramo(r,c,prev)}${textoAltura(r,p-1)}</div><div class="ficha recuerdo">/*@amplia*/${postal(r.id,p)||`/*@fin:amplia*/${escena(c.escena)}/*@amplia*/`}/*@fin:amplia*/<div class="cuerpo"><div class="imagen"><div class="fkicker">Imagen para recordar</div>${esc(c.imagen)}</div>${mas('Contame más',`<p>${esc(c.dato)}</p>`)}</div></div>${renderSello(r,p-1)}${!S.eco&&c.puerto?`<div class="puerto"><div class="fkicker">${V.enPuerto}</div><p>${esc(c.puerto)}</p><div class="bodega">${V.llevaba}: ${esc(c.carga)}</div></div>`:''}${renderMercado(r,p-1)}`}
  const f=S.eco&&S.eco.final;
  return `${globo(r,S.dicho||(r.companero&&r.companero.fin),'','dormido')}<p class="kicker">${botonVoz()}${V.kickerFin(r)}</p><h2>${esc(r.fin.nombre)}</h2><div class="ficha recuerdo">/*@amplia*/${postal(r.id,n+1)||`/*@fin:amplia*/${escena(r.fin.escena)}/*@amplia*/`}/*@fin:amplia*/<div class="cuerpo"><p>${esc(V.llegada(r))}</p></div></div>${f?`<div class="mercado"><div class="fkicker">Cuentas del viaje</div><p>${V.cuentas} ${f.fin}.${f.vend.length?` ${V.sobrante} ${esc(r.paradas[r.paradas.length-1].nombre)}: ${esc(f.vend.join(', '))}.`:''} ${f.gan>0?`Ganancia: ${f.gan}.`:'Sin ganancia esta vez.'}${r.fantasma!=null?` El ${V.mercader} llegó con ${r.fantasma}.`:''}</p><div class="bolsa">${ico('moneda')} Tesoro: ${num(P.tesoro||0)} monedas · ${rango(P.tesoro||0)}</div></div>`:''}${r.vehiculo?`<div class="nave">/*@amplia*/${laminaNave(r)||`/*@fin:amplia*/<svg class="barca mini" viewBox="-15 -15 30 20" aria-hidden="true">${glifo(r.vehiculo.tipo)}</svg>/*@amplia*/`}/*@fin:amplia*/<div class="fkicker">${esc(r.vehiculo.nombre)}</div><p>${esc(r.vehiculo.llegada)}</p></div>`:''}${mas(V.contame(r.contexto[1]),`<p>${esc(r.contexto[1].texto)}</p>`,r.contexto[1].titulo)}${bloqueFrase(r,true)}`}
...
function renderGuia(r){const g=S.guias[S.paso];if(!g||S.paso>=r.paradas.length)return '';
  const chips=g.opciones.map(i=>{let cls='chip';if(g.resp!=null){if(i===g.objetivo)cls+=' bien';else if(i===g.resp)cls+=' mal'}return `<button class="${cls}" onclick="responderGuia(${i})"${g.resp!=null?' disabled':''}>${esc(rotuloGuia(r,i))}</button>`}).join('');
...
function renderMercado(r,j){const V=r.vocab;const e=S.eco;if(!e||!r.carga.length)return '';const c=r.paradas[j];
  ...
  const slots=e.bodega.map((gi,bi)=>{const g=r.carga[gi],p=precio(r,g,j);return `<button class="item" onclick="vender(${bi})"><span class="ie">${/*@amplia*/bienPintado(r,gi)||/*@fin:amplia*/ico(g.i)}</span>${esc(g.n)}<span class="ip">${p>0?`vender · ${num(p)}`:'podrido · tirar'}</span></button>`}).join('')+Array(Math.max(0,3-e.bodega.length)).fill('<span class="item vacio">—</span>').join('');
  const abierto=S.llaves[j]!==false;
  const oferta=!abierto?'':r.carga.map((g,gi)=>({g,gi})).filter(x=>x.g.o===j).map(({g,gi})=>{const pista=g.d?`se pasa en ${g.d} ${g.d>1?V.puertosS:V.puertoS}`:g.m!=null?`se paga mejor en ${esc(oculta(r.paradas[g.m].nombre))}`:V.masAdelante;const no=e.monedas<g.b||e.bodega.length>=3;return `<button class="item${no?' no':''}" onclick="comprar(${gi})"><span class="ie">${/*@amplia*/bienPintado(r,gi)||/*@fin:amplia*/ico(g.i)}</span>${esc(g.n)}<span class="ip">comprar · ${num(g.b)}</span><small>${pista}</small></button>`}).join('');
...
function recordada(ok){const r=rioPor(S.rio),R=S.rec,n=r.paradas.length;if(R.i>=n||!R.revelada)return;marcar('ciudad:'+r.id+':'+R.i,ok,'recordar',R.pista==='nada'?2:1);R.ok.push(ok);R.dicho=azar(ok?VZ().recitarBien:VZ().recitarMal);R.i++;R.revelada=false;
  if(R.i>=n){const todas=R.ok.every(Boolean);if(R.ok.filter(Boolean).length>=n-1)hecha(r,'recitar');R.dicho=VZ().recitarFin[todas?0:R.ok.filter(Boolean).length>=n-1?1:2];if(S.eco&&todas){R.premio=R.pista==='nada'?5:R.pista==='iniciales'?3:1;P.tesoro=(P.tesoro||0)+R.premio;R.dicho+=` +${R.premio} monedas al tesoro.`;guardar()}}
  render(false)}
...
function renderOrden(r){const V=r.vocab,o=S.orden,U=unidades(r),n=U.length;
  ...
  const fin=o.listo?`<div class="nota${o.errores>1?' no':''}">${o.errores===0?V.ordenBien+(o.premio?` +${o.premio} monedas al tesoro.`:''):o.errores===1?'Un error: cuenta como acierto, pero repasalo.'+(o.premio?` +${o.premio} monedas al tesoro.`:''):V.ordenMal(o.errores)}</div>`:'';
  return `${globo(r,o.dicho)}<p class="kicker">${botonVoz()}Ordenar</p><p class="pregunta">${r.tramos?V.tocaTramos(n):V.tocaEnOrden(n)}</p>${seq}${o.listo?'':chips}${pista}${fin}`}
function renderQuiz(){const Q=S.quiz,n=Q.qs.length;
  if(Q.i>=n){const k=nivelQuiz(Q),msg=MSG_QUIZ(vocabQuiz())[k];
    ...
    return `${rm?globo(rm,VZ().resultado[k]):''}<div class="resultado"><p class="kicker">${botonVoz()}${esc(Q.titulo)}</p><h2>${Q.aciertos} de ${n}</h2><p>${msg}</p></div>`}
  ...
  const nota=resp!=null?`<div class="nota${resp===q.correcta?'':' no'}">${esc(q.nota)}</div>`:'';
  const rq=Q.modo==='rio'?rioPor(S.rio):(resp!=null?rioPor(q.rio):null);
  return `${rq?globo(rq,Q.dicho):globoNeutro(Q.dicho)}<p class="kicker">${botonVoz()}Pregunta ${Q.i+1} de ${n} · ${esc(Q.titulo)}</p>${q.escena?/*@amplia*/(postal(q.rio,q.ciudad+1,'quiz')||/*@fin:amplia*/escena(q.escena,'quiz')/*@amplia*/)/*@fin:amplia*/:''}<p class="pregunta">${esc(q.texto)}</p><div class="opciones">${ops}</div>${nota}`}
function render(scroll){voz.callar();GLOBO=false;const pie=document.getElementById('pie'),hPie=renderPie();if(pie){pie.hidden=!hPie;pie.innerHTML=hPie}if(document.body&&document.body.classList)document.body.classList.toggle('con-pie',!!hPie);const arriba=document.getElementById('arriba');if(arriba)arriba.classList.toggle('portada',S.pantalla==='inicio'||S.pantalla==='pasaporte');if(document.body&&document.body.classList)document.body.classList.toggle('nino',esNino());
  ...
  else if(S.pantalla==='pasaporte'){cab.innerHTML=cabecera('Pasaporte',`${num(totalSellos())} sellos · ${num(RUTAS.filter(completo).length)} ${VJ().completos}`,true);panel.innerHTML=renderPasaporte();renderMapa(S.zona?{modo:'zona',zona:S.zona,tocar:true}:{modo:'mundo',tocar:true})}
  else if(S.pantalla==='rio'){const r=rioPor(S.rio);cab.innerHTML=cabecera(r.nombre,r.vocab.cabecera(r),true);
    panel.innerHTML=(S.tab==='descender'?renderDescender(r):S.tab==='ordenar'?renderOrden(r):S.tab==='recitar'?renderRecitar(r):S.tab==='trazar'?renderTrazar(r):renderQuiz());renderMapa(focoRio(r))}
  else{cab.innerHTML=cabecera(esc(S.quiz.titulo),S.quiz.modo==='repaso'?'Lo que ya toca volver a ver':VJ().retoSub,true);panel.innerHTML=renderQuiz();renderMapa(focoQuiz(S.quiz.qs[S.quiz.i],null))}
  ...
  acomodarPie();if(scroll!==false)window.scrollTo(0,0)}
/* barra fija al pie: las acciones de la pantalla (guía y Zarpar, Revelar, Siguiente…) y, en la pantalla de ruta, el riel de
   viaje (Bajar · Ordenar · Recitar · Preguntar) con marcas de lo hecho; en el inicio y el pasaporte no hay barra */
const botones=(a,b)=>`<div class="botones">${a||'<span></span>'}${b||''}</div>`;
function etapasHechas(r){const e=(P.etapas||{})[r.id]||{};return{descender:!!P.vistos[r.id],ordenar:!!e.ordenar,recitar:!!e.recitar,trazar:!!e.trazar,preguntar:!!e.preguntar}}
function hecha(r,k){P.etapas=P.etapas||{};(P.etapas[r.id]=P.etapas[r.id]||{})[k]=true;guardar()}
function renderRiel(r){const h=etapasHechas(r),V=r.vocab;return `<nav class="riel" aria-label="Etapas">${[['descender',V.bajar],['ordenar','Ordenar'],r.tipo==='rio'?['recitar','Recitar']:['trazar',V.trazar],['preguntar','Preguntar']].map(([k,t])=>`<button class="paso${S.tab===k?' aqui':''}${h[k]?' hecho':''}" onclick="setTab('${k}')"${S.tab===k?' aria-current="step"':''}><i aria-hidden="true">${h[k]?ico('ok'):''}</i><span>${t}</span></button>`).join('')}</nav>`}
function pieQuiz(){const Q=S.quiz;if(!Q)return '';const n=Q.qs.length;
  ...
  return ''}
function renderPie(){if(S.pantalla==='inicio'||S.pantalla==='pasaporte')return '';let acto='',riel='';
  if(S.pantalla==='rio'){const r=rioPor(S.rio),V=r.vocab,n=r.paradas.length,p=S.paso;riel=renderRiel(r);
    const atras=`<button class="btn sec" onclick="paso(-1)">‹ Atrás</button>`,atrasEnlace=`<button class="enlace" onclick="paso(-1)">‹ Atrás</button>`;
    if(S.tab==='descender'){const E=S.evento;
      if(E){if(E.resp!=null){const hacia=E.tramo<=n?r.paradas[E.tramo-1].nombre:V.entreFin;acto=botones('',`<button class="btn" onclick="continuarEvento()">Seguir hacia ${esc(hacia)} ›</button>`)}}
      else if(p>n)acto=botones(atras,`<button class="btn" onclick="setTab('ordenar')">Ordenar de memoria ›</button>`);
      else{const g=S.guias[p],hayGuia=g&&p<n;
        if(!hayGuia)acto=botones(p>0?atras:'',`<button class="btn" onclick="paso(1)">${p===n?V.llegarFin:'Siguiente'} ›</button>`);
        else if(g.resp==null)acto=botones(`<span class="fkicker">${V.proxima}</span>`,p>0?atrasEnlace:'')+renderGuia(r);
        else acto=botones(p>0?atrasEnlace:'',`<button class="btn" onclick="paso(1)">${V.avanzar} ${esc(r.paradas[g.objetivo].nombre)} ›</button>`)+renderGuia(r)}}
    else if(S.tab==='ordenar'){if(S.orden.listo)acto=botones(`<button class="btn sec" onclick="setTab('ordenar')">Otra vez</button>`,`<button class="btn" onclick="setTab('preguntar')">Preguntar ›</button>`)}
    else if(S.tab==='recitar'){const R=S.rec;
      if(R.i>=n)acto=botones(`<button class="btn sec" onclick="setTab('recitar')">Otra vez</button>`,`<button class="btn" onclick="setTab('preguntar')">Preguntar ›</button>`);
      else if(!R.revelada)acto=botones('',`<button class="btn" onclick="revelar()">Revelar</button>`);
      else acto=botones(`<button class="btn sec" onclick="recordada(false)">No la sabía</button>`,`<button class="btn" onclick="recordada(true)">La sabía</button>`)}
    else if(S.tab==='trazar'){if(S.tra.listo)acto=botones(`<button class="btn sec" onclick="setTab('trazar')">Otra vez</button>`,`<button class="btn" onclick="setTab('preguntar')">Preguntar ›</button>`)}
    else acto=pieQuiz()}
  else acto=pieQuiz();
  return acto||riel?`<div class="dentro">${acto?`<div class="acto">${acto}</div>`:''}${riel}</div>`:''}
function acomodarPie(){const pie=document.getElementById('pie'),panel=document.getElementById('panel');if(!pie||!panel||!panel.style)return;panel.style.paddingBottom=pie.hidden?'':((pie.offsetHeight||0)+16)+'px'}
...
function focoRio(r){const n=r.paradas.length,f={modo:'rio',rio:r,tocar:false,etiquetas:new Set(),actual:null,halo:null};
  ...
  else if(S.tab==='ordenar'){const U=unidades(r);S.orden.seq.forEach(u=>{for(let i=U[u].desde;i<=U[u].hasta;i++)f.etiquetas.add(i)});if(S.orden.listo)for(let i=0;i<n;i++)f.etiquetas.add(i);Object.assign(f,estacionada(r))}
  else if(S.tab==='trazar')return focoTrazar(r);
...
/* caja de vista para unos puntos: se ajusta a la proporción del mapa desplegado (ancho/altoDes) y, si el mapa está plegado
   (alto < altoDes), recorta esa misma vista a la altura plegada centrando el punto de foco: misma escala, menos cielo, nunca
   un mapa encogido. El SVG lleva preserveAspectRatio slice, así que durante la transición de altura también se ve un recorte. */
function vbPara(pts,minW,margen,foco,dims){const m=$('#mapa'),ancho=(dims&&dims.ancho)||m.clientWidth||380,altoDes=(dims&&dims.des)||m.clientHeight||260,alto=(dims&&dims.alto)||altoDes,asp=ancho/altoDes;
  ...
  let w=(x1-x0)*margen,h=(y1-y0)*margen;const cx=(x0+x1)/2,cy=(y0+y1)/2;w=Math.max(w,minW);h=Math.max(h,minW/asp);if(w/h<asp)w=h*asp;else h=w/asp;
  if(alto<altoDes-1){const hc=w*alto/ancho,f=foco&&puntoFoco(foco),y=cy-h/2;let yc=(f?f[1]:cy)-hc/2;yc=Math.max(y,Math.min(y+h-hc,yc));return{x:cx-w/2,y:yc,w,h:hc}}
  return{x:cx-w/2,y:cy-h/2,w,h}}
function puntoFoco(foco){if(foco.modo!=='rio')return null;const b=foco.barca;if(b){if(b.poly)return proj(b.poly[b.t1>=b.t0?b.poly.length-1:0]);return proj(foco.rio.curso[b.a])}return foco.halo?proj(foco.halo):null}
/* medidas del mapa: ancho, alto desplegado y alto actual (plegado). El alto del CSS cambia con transición (plegar, entrar a un
   río, la barra al pie), así que clientHeight puede ir a medio camino: se miden los dos altos sin transición y después se
   restaura la animación desde la altura que tenía (FLIP), en vez de esperar los 300 ms del CSS */
function dimsMapa(){const a=document.getElementById('arriba'),m=$('#mapa'),ancho=m.clientWidth||380,h=m.clientHeight||260;
  const compacto=!!(a&&a.classList&&a.classList.contains('compacto'));if(!a||!m.style||typeof a.classList.remove!=='function')return{ancho,des:h,alto:h};
  m.style.transition='none';a.classList.remove('compacto');const des=m.clientHeight||h;a.classList.add('compacto');const ple=m.clientHeight||h;if(!compacto)a.classList.remove('compacto');
  m.style.height=h+'px';void m.offsetHeight;m.style.transition='';m.style.height='';return{ancho,des,alto:compacto?ple:des}}
...
function renderMapa(foco,inmediato){S.foco=foco;const capa=$('#capa'),m=$('#mapa'),dims=dimsMapa();let pts,vb;
  ...
  if(foco.rio){const r=foco.rio;
    r.paradas.forEach((c,i)=>{const q=proj(c.pos),on=foco.etiquetas.has(i);h+=`<circle class="ciudad${on?' on':''}" cx="${q[0].toFixed(1)}" cy="${q[1].toFixed(1)}" r="${f(3.5)}"/>`;if(foco.trazar)h+=`<circle class="hitz" cx="${q[0].toFixed(1)}" cy="${q[1].toFixed(1)}" r="${f(16)}" onclick="trazarEn(${i})"/>`;
...
const ANIMALES={
elefante:'<path class="rabo" d="M-11.5 -0.5 q-2.5 2 -1.8 5.5"/><ellipse class="cuerpo" cx="-3" cy="0.5" rx="8.8" ry="6.5"/><path class="cuerpo" d="M-9.5 5 h3.6 v5 h-3.6 Z M-2.5 5 h3.6 v5 h-3.6 Z"/><circle class="cuerpo" cx="7" cy="-3.5" r="5"/><path class="cuerpo" d="M10.8 -1.5 Q14 2 13 7.6 Q12.5 9.2 11.2 8.4 Q12 4 9.8 0.8 Z"/><ellipse class="cuerpo" cx="3.6" cy="-2.8" rx="2.6" ry="3.9"/><ellipse class="claro" cx="3.6" cy="-2.8" rx="1.2" ry="2.4"/><path class="dientes" d="M9.6 0.4 q1.9 1.4 3.2 1.1"/><circle class="oscuro" cx="8.8" cy="-4.6" r="1"/>',
beluga:'<path class="cuerpo" d="M-10.5 0.5 L-14 -3 L-12.2 0.5 L-14 4 Z"/><path class="cuerpo" d="M-11 0.2 Q-6 -4.4 1 -5 Q5.6 -5.4 8.6 -6.2 Q13 -6 13.6 -1.4 Q11.5 3.2 4 4 Q-3 4.6 -11 1.4 Z"/><path class="claro" d="M-6 2.6 Q2 4.6 10.5 2.2 Q2 3 -6 2.6 Z"/><path class="cuerpo" d="M0.5 3.9 l1.4 3.6 h3.2 l-1.2 -3.3 Z"/><circle class="oscuro" cx="9.6" cy="-1.6" r="1"/><path class="bigote" d="M11 1.3 q1.3 1 2.5 0"/>',
ornitorrinco:'<path class="oscuro" d="M-8 -1.4 L-13.6 -3.2 Q-14.6 1.5 -13.2 4.8 L-8 3.4 Z"/><ellipse class="cuerpo" cx="-1" cy="1" rx="8.4" ry="4.8"/><path class="cuerpo" d="M-4.5 4.6 l-2.6 3.6 h5 Z M3 4.6 l-2.2 3.6 h5 Z"/><circle class="cuerpo" cx="6.6" cy="-0.6" r="3.4"/><path class="acento" d="M8.6 -2 L14 -1.4 Q14.7 0 14 1.2 L8.6 1.8 Z"/><circle class="oscuro" cx="7.2" cy="-2" r="0.9"/>',
camello:'<path class="rabo" d="M-11 0.5 Q-13.5 2 -13 5.5"/><path class="cuerpo" d="M-11 2.5 Q-11.8 -2 -8 -3 Q-6.6 -9 -3 -4.6 Q-1.4 -9 2.2 -4.2 Q6.4 -3.6 7 -0.5 L7 3 L-11 3 Z"/><path class="acento" d="M-4.8 -4.2 h3.6 v2.6 h-3.6 Z"/><path class="cuerpo" d="M6 -1 L8.6 -8.4 L11.8 -9.4 L13.4 -7.6 L10.6 -6.2 L9.6 -0.4 Z"/><path class="cuerpo" d="M-9 3 h2.6 v6 h-2.6 Z M-5 3 h2.6 v6 h-2.6 Z M1 3 h2.6 v6 h-2.6 Z M5 3 h2.6 v6 h-2.6 Z"/><circle class="oscuro" cx="11.3" cy="-8" r="0.8"/><path class="bigote" d="M12.4 -7 q0.8 0.6 1.2 0"/>',
caballo:'<path class="rabo" d="M-10 -2 Q-13.8 0 -13 4.8"/><path class="cuerpo" d="M-10 1.5 Q-10.6 -4.2 -5 -4.2 L4 -4.2 Q7.8 -4.2 7.8 -1 L7.8 2 L-10 2 Z"/><path class="acento" d="M-3.5 -4.2 h5.5 v2.8 h-5.5 Z"/><path class="cuerpo" d="M5.5 -3 L8.4 -10.2 L12 -11.6 L14 -9.6 L11 -8 L10 -3 Z"/><path class="oscuro" d="M6.4 -4.4 Q9.4 -9.8 8.4 -12.8 Q7.2 -9.4 4.6 -5.4 Z"/><path class="cuerpo" d="M-8 2 h2.6 v6 h-2.6 Z M-4.4 2 h2.6 v6 h-2.6 Z M1.6 2 h2.6 v6 h-2.6 Z M5 2 h2.6 v6 h-2.6 Z"/><circle class="oscuro" cx="11.6" cy="-9.8" r="0.8"/><circle class="oscuro" cx="13.4" cy="-9.2" r="0.4"/>',
jirafa:'<path class="rabo" d="M-9 0 Q-11.5 1.5 -11 4.6"/><path class="cuerpo" d="M-9 1 Q-9.4 -3.4 -5 -3.4 L3 -3.4 Q6.4 -3.4 6.4 0 L6.4 2 L-9 2 Z"/><path class="cuerpo" d="M3.4 -3 L5 -11.4 L8.2 -12.2 L9.2 -10 L7.6 -2 Z"/><path class="oscuro" d="M-6.4 -2.2h2v2h-2zM-2.4 -1.4h2v2h-2zM1 -2.6h2v2h-2zM-4.6 0.6h1.2v1.2h-1.2z"/><path class="cuerpo" d="M-7 2 h2.2 v6.5 h-2.2 Z M-4 2 h2.2 v6.5 h-2.2 Z M1 2 h2.2 v6.5 h-2.2 Z M4 2 h2.2 v6.5 h-2.2 Z"/><circle class="cuerpo" cx="8.6" cy="-11.2" r="2.3"/><path class="cuerpo" d="M9.8 -12 L13 -11 L9.8 -9.8 Z"/><path class="acento" d="M7.4 -13.3 v-1.2 M9.8 -13.5 v-1.2" style="stroke:var(--dorado);stroke-width:1.3;fill:none;stroke-linecap:round"/><circle class="oscuro" cx="8.9" cy="-11.6" r="0.7"/>',
dromedario:'<path class="rabo" d="M-11 0.5 Q-13.5 2 -13 5.5"/><path class="cuerpo" d="M-11 2.5 Q-11.8 -2 -8 -3.4 Q-5 -10 0.4 -4.2 Q5.4 -3.6 7 -0.5 L7 3 L-11 3 Z"/><path class="acento" d="M-5.8 -5 h4.2 v2.6 h-4.2 Z"/><path class="cuerpo" d="M6 -1 L8.6 -8.4 L11.8 -9.4 L13.4 -7.6 L10.6 -6.2 L9.6 -0.4 Z"/><path class="cuerpo" d="M-9 3 h2.6 v6 h-2.6 Z M-5 3 h2.6 v6 h-2.6 Z M1 3 h2.6 v6 h-2.6 Z M5 3 h2.6 v6 h-2.6 Z"/><circle class="oscuro" cx="11.3" cy="-8" r="0.8"/><path class="bigote" d="M12.4 -7 q0.8 0.6 1.2 0"/>',
bonobo:'<path class="cuerpo" d="M-5 0.5 Q-11.5 3 -10 9.4 L-7.6 9 Q-8.4 4.4 -3.4 2.6 Z"/><path class="cuerpo" d="M3.6 1.2 Q9.8 3.8 8.8 9.4 L6.4 9 Q7 5 2.6 3.4 Z"/><ellipse class="cuerpo" cx="-0.8" cy="3.2" rx="6.6" ry="5.6"/><path class="cuerpo" d="M2.4 7.4 l1.2 4 h2.8 l-1.6 -4 Z M-5 7.4 l-1.6 4 h2.8 l1.2 -4 Z"/><circle class="cuerpo" cx="-2.2" cy="-5.2" r="1.7"/><circle class="cuerpo" cx="3" cy="-5.2" r="5.2"/><circle class="oscuro" cx="4" cy="-4.6" r="3.3"/><circle class="claro" cx="2.9" cy="-5.4" r="0.8"/><circle class="claro" cx="5.3" cy="-5.4" r="0.8"/><path class="dientes" d="M2.8 -2.8 q1.3 1 2.6 0"/><path class="bigote" d="M1 -9.8 Q3 -12.4 6.2 -10.6"/>',
carpa:'<path class="cuerpo" d="M-10.5 0 L-14 -5 L-12.2 0 L-14 5 Z"/><path class="acento" d="M-4.5 -5.4 Q-1 -11.4 5.2 -6.6 Z"/><path class="cuerpo" d="M-11 0 Q-6 -6.2 2 -6.2 Q10 -6.2 13.6 0 Q10 6.2 2 6.2 Q-6 6.2 -11 0 Z"/><path class="claro" d="M-3.4 6 Q-1.4 9.2 3.2 6.2 Z"/><path class="bigote" d="M-2.6 -3 q1.5 1.6 3 0 M1.4 -3 q1.5 1.6 3 0 M-0.6 0 q1.5 1.6 3 0 M3.4 0 q1.5 1.6 3 0 M-2.6 3 q1.5 1.6 3 0 M1.4 3 q1.5 1.6 3 0"/><circle class="oscuro" cx="8.8" cy="-2" r="1.1"/><path class="bigote" d="M12.6 1.2 L14.6 3 M12.6 2.2 L14 4.6"/>',
hipo:'<ellipse class="cuerpo" cx="-2.5" cy="1" rx="9.4" ry="6.6"/><path class="cuerpo" d="M-9 5.6 h3.6 v4 h-3.6 Z M-1 5.6 h3.6 v4 h-3.6 Z"/><path class="cuerpo" d="M3.5 -4.2 Q13.8 -5.2 13.8 1 Q13.8 5.4 5 5.4 Z"/><circle class="cuerpo" cx="4.4" cy="-5.6" r="1.9"/><circle class="cuerpo" cx="8.6" cy="-5.6" r="1.9"/><circle class="oscuro" cx="7.4" cy="-1.8" r="1.1"/><circle class="oscuro" cx="12" cy="0.6" r="0.6"/><circle class="oscuro" cx="12" cy="2.4" r="0.6"/><path class="bigote" d="M8.4 3.6 q2 1.2 3.8 0"/>',
delfin:'<path class="cuerpo" d="M-10.5 1 L-14 -2.6 L-12.4 1 L-14 4.2 Z"/><path class="cuerpo" d="M-1.6 -3.8 Q0.6 -7.4 3.6 -3.6 Z"/><path class="cuerpo" d="M-11 1 Q-7 -3.6 0 -4 Q6.4 -4.4 10.6 -2.6 L14.2 -1.4 L14.2 -0.2 L10.6 0.4 Q6.4 2.6 0 2.6 Q-7 2.6 -11 1 Z"/><path class="cuerpo" d="M1.4 2.4 l1.8 3.6 l3.6 -3.2 Z"/><circle class="oscuro" cx="8.4" cy="-1.8" r="1"/><path class="bigote" d="M10.8 0.2 q1.2 0.8 2.4 0.2"/>',
marsopa:'<path class="cuerpo" d="M-10.5 1.4 L-14 -1.6 L-12.4 1.5 L-14 4.6 Z"/><path class="cuerpo" d="M-11 1.4 Q-6 -4.2 1 -4.2 Q9 -4.2 13.4 -0.6 Q9 3.2 1 3.2 Q-6 3.2 -11 1.4 Z"/><path class="claro" d="M-4 2.8 Q2 4.4 9 2 Q2 2.4 -4 2.8 Z"/><path class="cuerpo" d="M2 2.8 l1.8 3.2 l3.6 -2.8 Z"/><circle class="oscuro" cx="9.2" cy="-1.6" r="1"/><path class="bigote" d="M10.6 1 q1.5 1 3 0"/>',
rana:'<ellipse class="cuerpo" cx="-8" cy="4.5" rx="3" ry="2.4"/><ellipse class="cuerpo" cx="8" cy="4.5" rx="3" ry="2.4"/><path class="pata" d="M-10.5 6.4 l-2 2.4 M-8.6 6.8 l-0.4 2.8 M-6.8 6.6 l1.4 2.4 M10.5 6.4 l2 2.4 M8.6 6.8 l0.4 2.8 M6.8 6.6 l-1.4 2.4"/><ellipse class="cuerpo" cx="0" cy="2" rx="9" ry="5.6"/><path class="claro" d="M-5 3 Q0 6.6 5 3 Q0 4.6 -5 3 Z"/><circle class="cuerpo" cx="-4" cy="-4.2" r="3.2"/><circle class="cuerpo" cx="4" cy="-4.2" r="3.2"/><circle class="claro" cx="-4" cy="-4.4" r="1.8"/><circle class="claro" cx="4" cy="-4.4" r="1.8"/><circle class="oscuro" cx="-3.6" cy="-4.4" r="0.9"/><circle class="oscuro" cx="4.4" cy="-4.4" r="0.9"/><path class="bigote" d="M-6 1.6 Q0 5.2 6 1.6"/>',
cisne:'<path class="claro" d="M-8.5 3 L-13 0.4 L-9.5 4.6 Z"/><path class="claro" d="M-9 3.2 Q-5 -2.6 3.4 -1.4 L7.6 2 Q1 6.6 -9 3.2 Z"/><path class="claro" d="M2.4 -0.6 Q3.8 -5.4 2.4 -8.2 Q1.2 -12.4 5.4 -12.4 Q9.6 -12.4 8.6 -8.4 L5.8 -8.2 Q7 -5 6.6 -0.4 Z"/><path class="oscuro" d="M8.2 -10.6 L9.6 -10.2 L8.4 -9.2 Z"/><path class="acento" d="M8.6 -10.6 L12.6 -9.4 L8.6 -8.4 Z"/><circle class="oscuro" cx="6.4" cy="-10.6" r="0.7"/>',
castor:'<path class="oscuro" d="M-7 3 Q-14.6 1.6 -13.2 7.4 Q-8.8 8.8 -6.6 4.4 Z"/><path class="bigote" d="M-12 4 l4 3.2 M-12.4 5.8 l2.8 2.4" style="stroke:var(--verde-claro);stroke-width:.5"/><ellipse class="cuerpo" cx="0.5" cy="1.5" rx="8.4" ry="6.2"/><path class="cuerpo" d="M-4 6.4 h3.4 v3.2 h-3.4 Z M2.4 6.4 h3.4 v3.2 h-3.4 Z"/><circle class="cuerpo" cx="6" cy="-6.6" r="1.5"/><circle class="cuerpo" cx="8.2" cy="-2.6" r="4.4"/><circle class="oscuro" cx="9.4" cy="-3.6" r="1"/><circle class="oscuro" cx="12.2" cy="-2" r="0.9"/><path class="claro" d="M10.2 0.2 h2.4 v2.4 h-2.4 Z"/><path class="bigote" d="M11.4 0.2 v2.4"/>',
gavial:'<path class="cuerpo" d="M-13 2.2 Q-8 -1.6 -2 -1.6 L4 -1.6 L6 -1.2 L13 -2.4 L13 -0.6 L6 0.6 L4 1.2 Q-2 3 -8 3 Q-11 3.2 -13 2.2 Z"/><path class="oscuro" d="M-9 -1.6 l1.2 -1.8 1.2 1.8 Z M-6 -1.6 l1.2 -1.8 1.2 1.8 Z M-3 -1.6 l1.2 -1.8 1.2 1.8 Z"/><path class="cuerpo" d="M-8 3 h2.8 v2.6 h-2.8 Z M-1 2.8 h2.8 v2.6 h-2.8 Z"/><circle class="cuerpo" cx="3.4" cy="-2.6" r="1.5"/><circle class="oscuro" cx="3.7" cy="-2.8" r="0.7"/><circle class="cuerpo" cx="12.4" cy="-2" r="1.2"/><path class="dientes" d="M6.8 0.2 l.9 .9 .9 -.9 .9 .9 .9 -.9 .9 .9 .9 -.9"/>',
esturion:'<path class="cuerpo" d="M-10.5 0.5 L-14 -4.6 L-12 0 L-14 3.2 Z"/><path class="cuerpo" d="M-11 0.5 Q-5 -3.6 3 -3.2 Q9 -2.8 13.6 -0.6 Q9 2.2 3 2.6 Q-5 3.2 -11 0.5 Z"/><path class="oscuro" d="M-7 -2.8 l1.4 -2 1.4 2 Z M-3.4 -3.1 l1.4 -2 1.4 2 Z M0.2 -3.1 l1.4 -2 1.4 2 Z M3.8 -2.8 l1.4 -2 1.4 2 Z"/><path class="cuerpo" d="M2 2.5 l1.5 3 l3.2 -2.6 Z"/><circle class="oscuro" cx="8.8" cy="-1.2" r="0.9"/><path class="bigote" d="M11 1 l0.6 2 M12.2 0.8 l0.6 2"/>',
tortuga:'<path class="cuerpo" d="M-10.5 0.4 L-13.6 1.2 L-10.5 2.4 Z"/><path class="cuerpo" d="M-7 4 L-10.4 7.4 L-5.4 6.2 Z M5 4 L8.4 7.4 L3.4 6.2 Z"/><ellipse class="cuerpo" cx="-1" cy="0.2" rx="9.6" ry="5.2"/><path class="bigote" d="M-9.6 1.6 Q-1 3.6 7.6 1.6"/><circle class="oscuro" cx="-4" cy="-2" r="0.55"/><circle class="oscuro" cx="0" cy="-3" r="0.55"/><circle class="oscuro" cx="4" cy="-2" r="0.55"/><path class="cuerpo" d="M8 -1.6 Q11 -3.4 13.6 -2.2 L14.2 -1.2 Q12 0.4 8.4 1.2 Z"/><circle class="oscuro" cx="11.6" cy="-2" r="0.7"/>',
bagre:'<path class="cuerpo" d="M-10.5 0.5 L-14 -3.6 L-12.4 0.5 L-14 4.6 Z"/><path class="cuerpo" d="M-1 -4.6 L1.2 -8.4 L3.6 -4.4 Z"/><path class="cuerpo" d="M-11 0.5 Q-6 -4.6 2 -4.6 Q9 -4.6 13.2 -0.4 Q9 3.6 2 4 Q-6 4.2 -11 0.5 Z"/><path class="cuerpo" d="M3 3.6 l1.5 3 l3.2 -2.6 Z"/><circle class="oscuro" cx="8.8" cy="-1.8" r="0.9"/><path class="bigote" d="M10 1.6 q2 1.2 3.2 0.2 M12.2 -0.8 l2.4 -1.6 M12.4 1.4 l2.4 2"/>',
manati:'<path class="cuerpo" d="M-9 1 Q-14.4 -1 -13.8 3.6 Q-12.2 6.4 -8.4 3 Z"/><path class="cuerpo" d="M-9 1 Q-8 -5.6 0 -5.6 Q8 -5.6 12.8 -1.4 Q10.8 3.2 4 4.2 Q-3 5.2 -9 1 Z"/><path class="cuerpo" d="M3.2 4.2 l-1.4 3 h4.6 Z"/><circle class="oscuro" cx="9.4" cy="-2" r="0.9"/><circle class="oscuro" cx="12.4" cy="-1.4" r="0.4"/><path class="bigote" d="M11 0.4 q1.4 0.6 2.4 0 M12 1.2 l2 1"/>',
nutria:'<path class="cuerpo" d="M-12 4.4 Q-9 -1 -3 -2.6 Q4 -4.2 9 -2.2 Q12.4 -0.6 11.4 2 Q6 3.6 -2 3.6 Q-8 3.6 -12 6.8 Z"/><path class="cuerpo" d="M-4 3.4 h3 v3.2 h-3 Z M4 3.2 h3 v3.2 h-3 Z"/><circle class="cuerpo" cx="7" cy="-4.6" r="1.3"/><circle class="cuerpo" cx="10.4" cy="-4.8" r="1.3"/><circle class="cuerpo" cx="9" cy="-1.6" r="3.6"/><path class="claro" d="M6.6 0.4 Q9 -0.6 11.4 0.4 Q9 2.4 6.6 0.4 Z"/><circle class="oscuro" cx="10.4" cy="-2.6" r="0.9"/><circle class="oscuro" cx="12.4" cy="-1" r="0.8"/><path class="bigote" d="M11.6 0 l3 0.3 M11.6 0.8 l3 1.3"/>',
cocodrilo:'<path class="cuerpo" d="M-13 1.6 Q-6 -2.2 1 -2.2 L12.6 -2.5 Q13.8 -1 12.6 0.6 L2 1.6 Q-6 3.6 -13 4.2 Z"/><path class="oscuro" d="M-9 -1 l1.2 -1.8 1.2 1.8 Z M-6 -1.7 l1.2 -1.8 1.2 1.8 Z M-3 -2.1 l1.2 -1.8 1.2 1.8 Z M0 -2.2 l1.2 -1.8 1.2 1.8 Z"/><path class="cuerpo" d="M-8 4.2 h2.8 v2.6 h-2.8 Z M0 1.8 h2.8 v2.6 h-2.8 Z"/><circle class="cuerpo" cx="4.2" cy="-3.2" r="1.5"/><circle class="oscuro" cx="4.5" cy="-3.4" r="0.7"/><path class="dientes" d="M6 0.7 l.9 1.1 .9 -1.1 .9 1.1 .9 -1.1 .9 1.1 .9 -1.1"/>',
mono:'<path class="rabo" d="M-6 5.5 Q-13.4 5 -12.4 -1.6 Q-11.4 -6 -8 -5"/><path class="cuerpo" d="M-4.5 1 Q-9.8 3.6 -8.4 9 L-6.2 8.6 Q-7 5 -3 3 Z"/><path class="cuerpo" d="M3.4 2 Q8.6 4.6 7.8 9 L5.6 8.6 Q6.2 5.4 2.6 3.6 Z"/><ellipse class="cuerpo" cx="-0.8" cy="3.4" rx="6" ry="5.2"/><path class="cuerpo" d="M2.6 7.4 l1 4 h2.4 l-1.4 -4 Z M-4.4 7.4 l-1.4 4 h2.4 l1 -4 Z"/><circle class="cuerpo" cx="-0.6" cy="-4.4" r="1.5"/><circle class="cuerpo" cx="4" cy="-4.2" r="4.6"/><circle class="claro" cx="5" cy="-3.6" r="2.8"/><circle class="oscuro" cx="4.2" cy="-4.4" r="0.65"/><circle class="oscuro" cx="6" cy="-4.4" r="0.65"/><path class="bigote" d="M4.3 -2.4 q0.8 0.7 1.6 0"/>',
tiburon:'<path class="cuerpo" d="M-10.5 0 L-13.6 -5.6 L-11.6 -0.6 L-14 4 L-10 1 Z"/><path class="cuerpo" d="M-7 -2.6 L-6 -4.8 L-4.4 -2.4 Z"/><path class="cuerpo" d="M-1 -3.4 L1.6 -9.2 L5.2 -3.2 Z"/><path class="cuerpo" d="M-11 0 Q-5 -3.6 2 -3.6 Q9 -3.6 13.6 -0.6 Q9 2.6 2 3 Q-5 3.6 -11 0 Z"/><path class="claro" d="M-5 1.6 Q2 3 10 1 Q2 1.8 -5 1.6 Z"/><path class="cuerpo" d="M2 2.8 L3.6 6.6 L7.2 2.4 Z"/><path class="bigote" d="M5.2 -0.6 v2.2 M6.5 -0.8 v2.4 M7.8 -0.9 v2.4"/><circle class="oscuro" cx="9.8" cy="-1.6" r="0.9"/><path class="bigote" d="M11.2 1.2 q1 0.9 2 0.3"/>',
lapa:'<path class="cuerpo" d="M-3 2 L-12.6 9.6 L-9 2 Z"/><path class="oscuro" d="M-3.4 2.4 L-12.6 10.4 L-11.4 4.6 Z" fill-opacity=".5"/><path class="cuerpo" d="M-2 -6 Q4 -9.4 7.2 -5.2 Q9.4 0 4.2 3.2 Q0 5.2 -3.2 2.2 Z"/><path class="acento" d="M0.4 -4.2 Q-6.2 -3 -8.4 2.2 L-2 0.2 Z"/><circle class="claro" cx="6" cy="-5.2" r="2.2"/><circle class="oscuro" cx="6.6" cy="-5.4" r="0.8"/><path class="oscuro" d="M8.2 -7 Q12.8 -7 10.4 -2.2 L8.2 -4.4 Z"/><path class="pata" d="M1.5 3.4 v2.6 M3.4 3.2 v2.6"/>',
cangrejo:'<path class="pata" d="M-6 3 l-4 4 M-4 4 l-3 5 M4 4 l3 5 M6 3 l4 4"/><ellipse class="cuerpo" cx="0" cy="1" rx="7.2" ry="4.8"/><path class="cuerpo" d="M-7 0 L-12.4 -3 L-10.4 -6.4 L-9 -2 Z M7 0 L12.4 -3 L10.4 -6.4 L9 -2 Z"/><path class="acento" d="M-12.4 -3 L-13.6 -5.4 L-10.4 -6.4 Z M12.4 -3 L13.6 -5.4 L10.4 -6.4 Z"/><path class="pata" d="M-2.5 -2 v-1.6 M2.5 -2 v-1.6"/><circle class="claro" cx="-2.5" cy="-3.2" r="1.4"/><circle class="claro" cx="2.5" cy="-3.2" r="1.4"/><circle class="oscuro" cx="-2.5" cy="-3.2" r="0.65"/><circle class="oscuro" cx="2.5" cy="-3.2" r="0.65"/><path class="bigote" d="M-1.6 1.8 q1.6 1.2 3.2 0"/>'};
...
function glifo(t){/* vista lateral, proa a la derecha, origen en el centro del casco; ≈ 24 unidades de ancho, de y=-13 (arriba) a y=4 */
  switch(t){
    case 'latina':return '<path class="casco" d="M-11 -1 Q-6 -3 0 -3 Q8 -3 12.5 -1.5 L10 3 Q0 4 -9 3 Z"/><path class="linea" d="M-9 0.2 Q0 1.4 10.8 0.2" stroke-width=".6"/><path class="vela" d="M-4 -2.6 Q0 -9.4 7.2 -14 Q8.4 -8.6 10.4 -2.6 Z"/><path class="palo" d="M-5 -2 L7.6 -14.4" stroke-width="1"/><path class="casco" d="M7.6 -14.4 L10.6 -13.4 L7.6 -12.4 Z"/>';
    case 'junco':return '<path class="casco" d="M-12 -4 L-9 -1 L9 -1 L12 -4 L10 3 L-9 3 Z"/><path class="linea" d="M-8 0.6 h17" stroke-width=".6"/><path class="palo" d="M1.6 -1 L1 -14" stroke-width="1"/><path class="vela" d="M-2 -1 L-4.4 -13 L7.4 -12 L5.4 -1 Z"/><path class="linea" d="M-3.8 -10 L6.9 -9.2 M-3.2 -7 L6.4 -6.4 M-2.6 -4 L5.9 -3.6" stroke-width=".6"/><path class="casco" d="M1 -14 L4 -13 L1 -12 Z"/>';
    case 'vapor':return '<path class="casco" d="M-12 0 L12 0 L10.5 3 L-10.5 3 Z"/><path class="vela" d="M-9 -3.5 h18 v3.5 h-18 Z"/><path class="vela" d="M-6 -7 h12 v3.5 h-12 Z"/><path class="linea" d="M-7 -2 h2 M-3 -2 h2 M1 -2 h2 M5 -2 h2 M-4 -5.5 h2 M2 -5.5 h2" stroke-width=".8"/><path class="vela" d="M-2 -9.6 h4 v2.6 h-4 Z"/><path class="palo" d="M-4 -7 v-5 M4 -7 v-5" stroke-width="1.6"/><circle class="humo" cx="-4" cy="-13.2" r="1.4"/><circle class="humo" cx="4" cy="-13.2" r="1.4"/><circle class="humo" cx="0.5" cy="-14.4" r="1"/><circle class="palo" cx="-11.5" cy="0" r="3.2" stroke-width="1"/><path class="palo" d="M-11.5 -3.2 v6.4 M-8.3 0 h-6.4" stroke-width=".7"/>';
    case 'balsa':return '<path class="casco" d="M-11 -2.5 Q-12 -2.5 -12 -1.5 L-12 1.5 Q-12 2.5 -11 2.5 L11 2.5 Q12 2.5 12 1.5 L12 -1.5 Q12 -2.5 11 -2.5 Z"/><path class="linea" d="M-7 -2.5 v5 M-2.5 -2.5 v5 M2 -2.5 v5 M6.5 -2.5 v5" stroke-width=".6"/><circle class="fig" cx="4" cy="-6.6" r="1.7"/><path class="palo" d="M4 -4.9 v2.4" stroke-width="1.2"/><path class="palo" d="M0.5 -9.4 L8 -1.2" stroke-width=".9"/><path class="casco" d="M7 -2.4 L9.4 -0.2 L7.6 0.6 Z"/>';
    case 'canoa':return '<path class="casco" d="M-13.5 -1.6 Q-9 -3.6 0 -3.6 Q9 -3.6 13.5 -1.6 Q9 2 0 2.6 Q-9 2 -13.5 -1.6 Z"/><path class="linea" d="M-11 -0.6 Q0 0.6 11 -0.6" stroke-width=".6"/><circle class="fig" cx="4" cy="-6.6" r="1.7"/><path class="palo" d="M4 -4.9 v1.6" stroke-width="1.2"/><path class="palo" d="M0.8 -9.6 L7.4 0" stroke-width=".9"/><path class="casco" d="M6.4 -1.6 L8.8 0.6 L7 1.4 Z"/>';
    case 'barcaza':return '<path class="casco" d="M-13 -1 L13 -1 L12 3 L-12 3 Z"/><path class="linea" d="M-11 -1 v-2.4 h5.5 v2.4 M-4 -1 v-2.4 h5.5 v2.4" stroke-width=".8"/><path class="vela" d="M6 -1 v-5.4 h5.4 v5.4 Z"/><path class="linea" d="M7.4 -4.6 h2.6" stroke-width=".7"/><path class="palo" d="M12.4 -1 v-6" stroke-width=".8"/><path class="casco" d="M12.4 -7 L14.8 -6 L12.4 -5 Z"/>';
    case 'caravana':return '<path class="casco" d="M-9 0 Q-10 -6 -4 -7 Q0 -11 4 -7 Q8 -7 8 -2 L8 0 Z"/><path class="casco" d="M8 -2 L10 -10 L13 -11 L14 -9 L11 -8 L10 0 Z"/><path class="palo" d="M-7 0 L-7 4 M-3 0 L-3 4 M2 0 L2 4 M6 0 L6 4" stroke-width="1.2"/><path class="vela" d="M-5.4 -10.4 L1.4 -10.4 L2.4 -7 L-6.4 -7 Z"/><path class="linea" d="M-2 -10.4 L-2 -7" stroke-width=".6"/><path class="palo" d="M-9 -1 L-12 3" stroke-width=".8"/>';
    case 'jinete':return '<path class="casco" d="M-10 0 Q-11 -5.4 -5 -6.4 L3 -6.4 Q8.4 -6.4 8.8 -2 L8.8 0 Z"/><path class="casco" d="M7.4 -4 L9.8 -10.6 L13 -11.6 L14.4 -9.6 L11.6 -8 L10.6 -1 Z"/><path class="palo" d="M-7 0 L-8 4 M-4 0 L-3 4 M2 0 L1 4 M6 0 L7 4" stroke-width="1.2"/><path class="palo" d="M-10 -1 L-13 3" stroke-width=".8"/><path class="vela" d="M-3.4 -10 h4.4 v3.8 h-4.4 Z"/><circle class="fig" cx="-1.2" cy="-12.2" r="1.9"/><path class="palo" d="M1 -9.4 L4.4 -7.4" stroke-width="1.2"/><path class="palo" d="M4.4 -15 v9" stroke-width=".9"/>';
    case 'pie':return '<circle class="fig" cx="0" cy="-10.5" r="2.1"/><path class="palo" d="M0 -8.4 L0 -2 M0 -2 L-3 4 M0 -2 L3 4 M0 -7 L4 -4 M0 -7 L-3 -2" stroke-width="1.3"/><path class="palo" d="M5 -13 L5 4" stroke-width=".9"/><path class="casco" d="M-4.4 -8 L-1 -9 L-1 -4 L-4.4 -3 Z"/>';
    default:return '<path class="casco" d="M-11 -1 Q-6 -3 0 -3 Q7 -3 12 -1.5 L10 3 L-9 3 Z"/><path class="linea" d="M-9 0.4 Q0 1.6 10.4 0.4" stroke-width=".6"/><path class="palo" d="M-1 -3 v-11.4 M8 -3 v-8.6" stroke-width="1"/><path class="vela" d="M-6.2 -12.6 L4.2 -12.6 Q5.4 -8 4.2 -3.4 L-6.2 -3.4 Q-7.4 -8 -6.2 -12.6 Z"/><path class="vela" d="M5.4 -9.6 L11 -9.6 Q11.8 -6.6 11 -3.4 L5.4 -3.4 Q4.6 -6.6 5.4 -9.6 Z"/><path class="casco" d="M-1 -14.4 L2.6 -13.4 L-1 -12.4 Z"/>'}}
let animB=null;
function animarBarca(poly,t0,t1,px,tierra){const g=document.getElementById('barca'),m=document.getElementById('masc');if(!g)return;if(animB)cancelAnimationFrame(animB);
  ...
  if(t0===t1||reduce){poner(pos(t1));S.olas=false;return}
  const T=performance.now(),dur=1400;
  const step=now=>{let k=Math.min(1,(now-T)/dur);k=k<.5?2*k*k:1-Math.pow(-2*k+2,2)/2;const p=pos(t0+(t1-t0)*k);poner(p);dibujarEstela(p);if(k<1)animB=requestAnimationFrame(step);else{animB=null;if(estela)estela.setAttribute('class','estela fin');salpicar(p);if(S.olas){S.olas=false;olear()}}};
  animB=requestAnimationFrame(step)}

/* olas doradas: una barrida sobre el mar al llegar al final de la ruta (clase .va en #olas, se quita sola al terminar) */
function olear(){const o=document.getElementById('olas');if(!o||!o.classList)return;o.classList.remove('va');void o.offsetWidth;o.classList.add('va')}

/* ---------- arranque ---------- */
(function init(){cargarPerfiles();if(voz.soporte())window.speechSynthesis.getVoices();render(true)})();
window.addEventListener('resize',()=>{acomodarPie();if(S.foco)renderMapa(S.foco,true);const pf=document.getElementById('perfil');if(pf&&!pf.hidden&&S.pantalla==='rio')pf.innerHTML=perfilSVG(rioPor(S.rio))});
function plegarMapa(){const a=document.getElementById('arriba');a.classList.toggle('compacto');const b=document.querySelector('.plegar');b.textContent=a.classList.contains('compacto')?'▴':'▾';
  if(S.foco)renderMapa(S.foco,!a.classList.contains('compacto'))/* al plegar, la caja se anima (el alto del CSS va más rápido y manda); al desplegar salta ya, y el mapa solo destapa */}
