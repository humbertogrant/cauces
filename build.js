...
function usados(datos){/* claves que usan los datos del juego: glifos de animal, pictogramas de escena y tipos de vehículo */
  ...
  return{glifos:g,pictos:p,tipos:t}}
function recortar(motor,j,u){
  let m=motor,quitado={vocab:0,animales:0,pictos:0,glifos:0};
...
for(const [id,j] of Object.entries(JUEGOS)){
  ...
  console.log(`dist/${id}.html`,(html.length/1024).toFixed(0),'KB','· recortado del motor:',`${quitado.vocab} vocabularios, ${quitado.animales} animales, ${quitado.pictos} pictogramas, ${quitado.glifos} glifos`);if(html.length>500*1024)console.warn(`AVISO: dist/${id}.html pasa de 500 KB`)}
